...
private:
   const double side;
   public:
  Square ( const char  * const token[4]): side(atof(token[1]))
  {
  }
  void print() const;
};
...
public:
  Rectangle(const char * const token[4]):length(atof(token[1])), width(atof(token[2]))
  {
  }
  void print() const;
};
...
private:
  const double  radius;
public:
  Circle(const char * const token[4]):radius(atof(token[1]))
  {
  }
  void print() const;
};
...
private:
  const double  side;
public:
  Cube(const char * const token[4]):side(atof(token[1]))
  {
  }
  void print() const;
};
...
public:
  Prism(const char  * const token[4]):length(atof(token[1])),width(atof(token[2])),height(atof(token[3]))
  {
  }
  void print() const;
};
...
public:
  Cylinder(const char * const token[4]):radius(atof(token[1])),height(atof(token[2]))
  {
  }
  void print() const;
};
