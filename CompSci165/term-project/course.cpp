...
{
  int choice;
  cout << "\n\nPlease select from the following [1 - 5]:\n"
    << "[1] add\n[2] delete\n[3] list\n[4] sort\n[5] quit\n>>";
  cin >> choice;
  cin.ignore(1000,10);
  return choice;
...
char course::compareDCmp( const course* src)
{
  ...
  course tempSRC =*src;
  course tempDST = *this;
  toLower(tempSRC.nCourse);
  toLower(tempDST.nCourse);
...
//delete node
course* course::deleteNode(course* h, course& t) // header 
{ 
  int nList = 1;
  for(course* p=h;p;p=p->next)
  {
    cout <<   nList << ".) " << setw(15) << p->nCourse <<setw(7) << " "<< p->term
      << setw(6) <<  p->units <<  setw(8) << p->grade << endl;
    nList++;
  }//for
  ...
  while(true)
  {
    ...
    for(h,prev=0;nCount<=choice;prev=h,h=h->next,nCount++)
    {
      if(nCount==choice)
      {     
        if(choice==1)
